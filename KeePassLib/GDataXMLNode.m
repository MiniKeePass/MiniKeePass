...
- (id)initWithReadIO:(xmlInputReadCallback)ioread closeIO:(xmlInputCloseCallback)ioclose context:(void*)ioctx options:(unsigned int)mask error:(NSError **)error {
    self = [super init];
    if (self) {
        const char *baseURL = NULL;
        const char *encoding = NULL;
        
        xmlDoc_ = xmlReadIO(ioread, ioclose, ioctx, baseURL, encoding,
                  kGDataXMLParseOptions); // TODO(grobbins) map option values
        if (xmlDoc_ == NULL) {
            if (error) {
                *error = [NSError errorWithDomain:@"com.google.GDataXML"
